...
var nCage={
	...
	height: function (elem){
		return Math.max(elem['clientHeight'], elem['offsetHeight'], elem['scrollHeight']);
	},
	width: function (elem){
		return Math.max(elem['clientWidth'], elem['offsetWidth'], elem['scrollWidth']);
	},
	...
	handleImages: function (lstImgs, time){
		...
		for(var i=0; i<images.length; i++){
			let image=images[i];
			let src=image.src;
			if(image.dataset.__origSrc){
				src=image.dataset.__origSrc;
			}
			if(typeof nCage.srcList[src]==='undefined'){
				nCage.srcList[src]=true;
				let h=nCage.height(image);
				let w=nCage.width(image);
				//If image loaded
				if(h>0&&w>0){
					nCage.handleImg(image, lstImgs);
				}else{
		...
		//Keep replacing
		if(time>0&&lstImgs.length>0){
			setTimeout(function (){
				nCage.handleImages(lstImgs, time);
			}, time);
		}
	},
	//Replace one image
	handleImg: function (image, lstImgs){
		let h=nCage.height(image);
		let w=nCage.width(image);
		image.style.height=h+'px';
		image.style.width=w+'px';
		image.addEventListener('error', function (){
			nCage.handleBrokenImg(image, lstImgs);
		});
		nCage.setRandomImg(image, lstImgs);
	},
	//Set a random image from lstImgs to item
	setRandomImg: function (image, lstImgs){
		if(!image.dataset.__origSrc){
			image.dataset.__origSrc=image.src;
		}
		if(lstImgs.length>0){
			image.src=lstImgs[Math.floor(Math.random()*lstImgs.length)];
		}else{
			image.src=image.dataset.__origSrc
		}
	},
	//Removed broken image from lstImgs, run handleImg on item
	handleBrokenImg: function (image, lstImgs){
		var brokenImg=image.src;
...
nCage.handleImages(nCage.nCageImgs, 3000);
